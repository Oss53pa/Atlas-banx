...
// The package is not published to npm; it is vendored here (dependency-free,
// pure fetch) so the Vercel single-file build stays green. Keep in sync with
// oss53pa/atlas-studio-website:proph3t-client/src/types.ts.
// ============================================================================

/**
 * App identifier registered in the Atlas Studio catalogue (table `apps`,
 * visible in Admin → Applications). The core normalises aliases
 * (atlas-compta→atlas-fa, taxpilot→liasspilot). Use the catalogue id.
 */
...
export interface RunToolParams<TArgs = Record<string, unknown>> {
  /** Tool name from the central registry. */
  name: string;
  args: TArgs;
}
